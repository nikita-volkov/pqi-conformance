  ...
  describe "execPrepared" do
    it "executes a prepared statement" \conninfo ->
      differential adapter conninfo \connection -> do
        ...
        connection
          . execPrepared "conformance_stmt" [Just ("hello", Lq.Text), Just ("7", Lq.Text)] Lq.Text
    ...
    it "executes the unnamed prepared statement" \conninfo ->
      differential adapter conninfo \connection -> do
        ...
        connection
          . execPrepared "" [Just ("41", Lq.Text)] Lq.Text
          >>= traverse observeResult
    ...
    it "executes a zero-parameter statement" \conninfo ->
      differential adapter conninfo \connection -> do
        ...
        connection
          . execPrepared "conformance_no_params" [] Lq.Text
          >>= traverse observeResult
    ...
    it "binds null and binary parameters with a binary result" \conninfo ->
      differential adapter conninfo \connection -> do
        ...
        connection
          . execPrepared "conformance_binary" [Nothing, Just ("\NUL\1\255", Lq.Binary)] Lq.Binary
          >>= traverse observeResult

    it "rejects an unknown statement" \conninfo ->
      differential adapter conninfo \connection ->
        connection
          . execPrepared "conformance_missing" [] Lq.Text
          >>= traverse observeResult
