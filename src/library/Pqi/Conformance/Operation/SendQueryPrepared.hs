  ...
  describe "sendQueryPrepared" do
    it "executes a prepared statement asynchronously" \conninfo ->
      differential adapter conninfo \connection -> do
        _ <- Lq.sendPrepare connection "conformance_send_exec" "select $1 :: int4 * 2" Nothing
        _ <- drainResults connection
        sent <- Lq.sendQueryPrepared connection "conformance_send_exec" [Just ("21", Lq.Text)] Lq.Text
